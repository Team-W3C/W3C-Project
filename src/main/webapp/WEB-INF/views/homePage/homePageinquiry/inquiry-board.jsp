        ...
        <table class="inquiry-table">
            ...
            <c:choose>
                <c:when test="${empty list}">
                    <tr>
                        <td colspan="6" style="text-align: center; padding: 40px 0; color: #666;">
                            결과가 없습니다
                        </td>
                    </tr>
                </c:when>
        ...
        <c:choose>
            <c:when test="${empty keyword and empty category}">
                <c:if test="${pi.currentPage > 1}">
                    <a href="${pageContext.request.contextPath}/member/inquiry-board?cpage=${pi.currentPage - 1}" class="prev" aria-label="이전 페이지">← Previous</a>
                </c:if>
                <c:forEach var="i" begin="${pi.startPage}" end="${pi.endPage}">
                    <c:choose>
                        <c:when test="${i == pi.currentPage}"><a href="#" class="active">${i}</a></c:when>
                        <c:otherwise><a href="${pageContext.request.contextPath}/member/inquiry-board?cpage=${i}">${i}</a></c:otherwise>
                    </c:choose>
                </c:forEach>
                <c:if test="${pi.currentPage < pi.maxPage}">
                    <a href="${pageContext.request.contextPath}/member/inquiry-board?cpage=${pi.currentPage + 1}" class="next" aria-label="다음 페이지">Next →</a>
                </c:if>
            </c:when>
            <c:otherwise>
                <c:if test="${pi.currentPage > 1}">
                    <a href="${pageContext.request.contextPath}/member/inquiry-board?cpage=${pi.currentPage - 1}&keyword=${keyword}&category=${category}" class="prev">← Previous</a>
                </c:if>
                <c:forEach var="i" begin="${pi.startPage}" end="${pi.endPage}">
                    <c:choose>
                        <c:when test="${i == pi.currentPage}"><a href="#" class="active">${i}</a></c:when>
                        <c:otherwise><a href="${pageContext.request.contextPath}/member/inquiry-board?cpage=${i}&keyword=${keyword}&category=${category}">${i}</a></c:otherwise>
                    </c:choose>
                </c:forEach>
                <c:if test="${pi.currentPage < pi.maxPage}">
                    <a href="${pageContext.request.contextPath}/member/inquiry-board?cpage=${pi.currentPage + 1}&keyword=${keyword}&category=${category}" class="next">Next →</a>
                </c:if>
            </c:otherwise>
        </c:choose>
    </nav>
